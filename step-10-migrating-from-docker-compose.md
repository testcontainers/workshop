...
and are wondering are wondering how to get started from here?
...
```java
@Container
static DockerComposeContainer composeContainer = new DockerComposeContainer(new File("docker-compose-tc.yml"))
...
Instead of defining the necessary services in the `docker-compose.yml` file, we will not declare them as Java objects.
...
(we don't have to rebuild the whole application and image) or much easier debugging from the JVM.
...
And we can use the `@DynamicPropertySource` to comfortably configure the whole Spring-Boot application:
